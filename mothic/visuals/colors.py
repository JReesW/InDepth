from pygame.math import lerp as _lerp
...
def cap(color: Color) -> Color:
    """
    Cap the channels of a given color within the range 0-255, in case a calculation produces numbers outside of this range
    """
    return tuple(min(max(c, 0), 255) for c in color)
...
def alter(color: Color, degree: float) -> Color:
    ...
    TODO: make sure this doesn't change the alpha
    """
    return cap(round(c * degree) for c in color)


def lerp(col1: Color, col2: Color, value: float) -> Color:
    return tuple(int(_lerp(a, b, value)) for a, b in zip(col1, col2))
